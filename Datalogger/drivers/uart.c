...
void UART_Init(LPUART_Type *base, LPUART_InitStruct_t *LPUART_InitStruct)
{
	...
	if (base == LPUART0)
	{
    	...
    	CLOCK_enable(PCC_PORTC_INDEX);
        PORT_Pin_Init(PORTB, 0, &config);
        PORT_Pin_Init(PORTB, 1, &config);
	}
	else if (base == LPUART1)
	{
		CLOCK_enable(PCC_LPUART1_INDEX);
    	CLOCK_enable(PCC_PORTC_INDEX);
        PORT_Pin_Init(PORTC, 6, &config);
        PORT_Pin_Init(PORTC, 7, &config);
	}
	else if (base == LPUART2)
	{
		CLOCK_enable(PCC_LPUART2_INDEX);
    	CLOCK_enable(PCC_PORTD_INDEX);
        PORT_Pin_Init(PORTD, 6, &config);
        PORT_Pin_Init(PORTD, 7, &config);
	}
	...
	/*Set baudrate*/
	UART_SetBaudrate(base, LPUART_InitStruct->BaudRate, CLOCK_getFreq());
