...
bool CircularQueue_IsFull(const CircularQueue_t *queue){
	if( ((queue->head + 1) % QUEUE_SIZE) == queue->tail) return true;
	return false;
}
...
bool CircularQueue_Enqueue(CircularQueue_t *queue, uint8_t data){
    if (CircularQueue_IsFull((const CircularQueue_t *)queue)) {
        return false;
    }
...
bool CircularQueue_Dequeue(CircularQueue_t *queue, uint8_t *data){
    if (CircularQueue_IsEmpty((const CircularQueue_t *)queue)) {
        return false;
    }
